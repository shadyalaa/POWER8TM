	...
	while(rot_budget > 0){ \
		...
		if(IS_LOCKED(single_global_lock)){ \
			...
			continue; \
                ...
                /*crot_readset = (readset_t *)malloc(sizeof(readset_t)); \
                readset_item_t *item = (readset_item_t *)malloc(sizeof(readset_item_t)); \
                item->addr = -1; \
                item->next = NULL; \
                crot_readset->head = item; */\
		rs_counter = 0; \
		offset=0; \
...
# define TOUCH_REVALIDATION(){ \
	...
	int i; \
		...
		if(!index){ \	
			for(i=0; i<8; i++){ \
            			temp_add = crot_readset[index+i] | ( temp_add << 8) ; \
			} \
			/*printf("base add is %p\n",temp_add); */\
			temp = *(long *)temp_add; \
			offset = temp_add; \
			index += 8; \
		} \
		else{ \
			step = crot_readset[index] & 0x70; \
                        step >>= 4; \
			/*if(step == 7){ \
				temp_add = 0; \
			} \
			...
			for(i = 1; i < step+1; i++){ \
                                temp_add = crot_readset[index+i] | ( temp_add << 8) ; \
				/*if(step==7)  \
				printf("\t\tchar is %x\n",crot_readset[index+i]&0xff); */\
			} \
			temp_add  = crot_readset[index] | ( temp_add << 8) ; \
			/*printf("\tstep: %d add is %p\n",step,temp_add); */\
                        temp = *(long *)temp_add; \
			index += step+1; \
		} \
	} \
...
# define TM_BEGIN_EXT(b,ro) { \
	...
	local_exec_mode = 0; \
	rs_mask_2 = 0xffffffffffff0000; \
	rs_mask_4 = 0xffffffff00000000; \ 
...
#define LOG_ADDRESS(var)             				curr_addr = (long)var; \
								p = (char *)&curr_addr; \
                                                                /*mcurr_addr = curr_addr & rs_mask_2; \
                                                                moffset = offset & rs_mask_2; \
                                                                if(!(mcurr_addr ^ moffset)){ \
									crot_readset[rs_counter] = p[7] | 0x10; \
									crot_readset[rs_counter+1] = p[6]; \
									rs_counter += 2; \
                                                                } \
								else{ */\
									mcurr_addr = curr_addr & rs_mask_4; \
                	                                                if(!(mcurr_addr ^ moffset)){ \
										crot_readset[rs_counter++] = p[7] & 0x80 | 0x30; \
	                                                                        crot_readset[rs_counter++] = p[4]; \
										crot_readset[rs_counter++] = p[5]; \
										crot_readset[rs_counter++] = p[6]; \
									} \
									else{ \
										offset = (long)var; \
										moffset = offset & rs_mask_4; \
										crot_readset[rs_counter++] = p[7] & 0x80 | 0x70; \
                                                                                crot_readset[rs_counter++] = p[0]; \
                                                                                crot_readset[rs_counter++] = p[1]; \
                                                                                crot_readset[rs_counter++] = p[2]; \
                                                                                crot_readset[rs_counter++] = p[3]; \
                                                                                crot_readset[rs_counter++] = p[4]; \
                                                                                crot_readset[rs_counter++] = p[5]; \
                                                                                crot_readset[rs_counter++] = p[6]; \
										/*printf("\t\t%p\tchar is %x\n",curr_addr,crot_readset[rs_counter+7]&0xff); */\
									} \
