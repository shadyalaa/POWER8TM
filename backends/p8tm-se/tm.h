...
# define QUIESCENCE_CALL_ROT_SINGLE(){ \
        int num_threads = global_numThread; \
        int index; int state;\
        register long temp; \
        for(index=0; index < num_threads; index++){ \
		...
		state = temp & 7; \
		if((temp == 3) || (temp == 1)){ \
                        while(counters[index].value==temp) \
                        cpu_relax(); \
                }\
		rmb(); \
        } \
...
# define TOUCH_REVALIDATION(){ \
	int index = 0; \
	long temp; \
	long base = 0; /*offset & 0xFFFFFFFF00000000;*/ \
	int* base_p = (int*)&base; \
        while(index<rs_counter){ \
		base_p[1] = irot_readset[index++]; \
		/*base = base | (irot_readset[index++] & 0x00000000FFFFFFFF) ; */\
		/*printf("temp_add: %p\n",temp_add); */\
		/*(step = temp_add & 0x70;*/ \
		if((base_p[1]& 0x70) == 0x70){ \
			base_p[0] = irot_readset[index++];/* & 0x00000000FFFFFFFF;*/ \
			/*base = base << 32; \
			temp_add = (base << 32) | temp_add;*/ \
			/*printf("new base: %p\t%p\t%p\n",base,irot_readset[index-1],irot_readset[index-2]); */\
			temp = *(long *)base; \
		} \
		else {\
                        /*temp_add = base | temp_add;*/ \
			/*printf("address: %p\t%p\n",irot_readset[index-1],base); */\
                        temp = *(long *)base; \
		} \
	} \
};
...
#define LOG_ADDRESS(var)             			if(var){ \
								...
								ip = (int *)&curr_addr; \
								register long mcurr_addr = curr_addr & rs_mask_4; \
                	                                        if(!(mcurr_addr ^ moffset)){ \
									irot_readset[rs_counter++] = (ip[1] & 0xFFFFFF80) | 0x30; \
								} \
								else{ \
									offset = (long)var; \
									moffset = offset & rs_mask_4; \
									irot_readset[rs_counter++] = (ip[1] & 0xFFFFFF80) | 0x70; \
									irot_readset[rs_counter++] = ip[0]; \
								} \
							} \
