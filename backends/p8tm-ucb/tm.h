...
#  define TM_STARTUP(numThread, bId) ucb_start();
#  define TM_SHUTDOWN(){ \
...
# define ACQUIRE_READ_LOCK() { \
	while(1){ \
		...
		rmb(); \
		if(IS_LOCKED(single_global_lock) || !allow_rots_ros){ \
...
# define RELEASE_READ_LOCK() counters[local_thread_id].value+=7; stats_array[local_thread_id].read_commits++; //printf("committed as uninstrumented reads\n");
...
# define RELEASE_WRITE_LOCK(){ \
	if(!local_exec_mode){ \
		...
		__TM_end(); \
		/*counters[local_thread_id].value += 2; */\
